...
class GameScene: SKScene ,SKPhysicsContactDelegate, AVAudioPlayerDelegate{
    ...
    override func didMove(to view: SKView) {
        ...
        scrollNode.addChild(panNode)   // 追加
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
        ...
        else if (contact.bodyA.categoryBitMask & panCategory) == panCategory || (contact.bodyB.categoryBitMask & panCategory) == panCategory {
            // スコア用の物体と衝突した
            print("itemScoreUp")
            itemScore += 1
            ...
            //play()
            playSound()
            self.panNode.removeAllChildren()
        }
        else {
            // 壁か地面と衝突した
            print("GameOver")
    ...
    func setupPan() {
        // 壁の画像を読み込む
        let panTexture = SKTexture(imageNamed: "pan")
        panTexture.filteringMode = .linear
        
        // 移動する距離を計算
        let movingDistancePan = CGFloat(self.frame.size.width + panTexture.size().width)
        
        // 画面外まで移動するアクションを作成
        let movePan = SKAction.moveBy(x: -movingDistancePan, y: 0, duration:4.0)
        
        // 自身を取り除くアクションを作成
        let removePan = SKAction.removeFromParent()
        
        // 2つのアニメーションを順に実行するアクションを作成
        let panAnimation = SKAction.sequence([movePan, removePan])
        
        // 壁を生成するアクションを作成
        let createPanAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let pannode = SKNode()
            pannode.position = CGPoint(x: self.frame.size.width + panTexture.size().width / 2, y: 0.0)
            pannode.zPosition = -50.0 // 雲より手前、地面より奥
            ...
            let random_y_range = self.frame.size.height / 4
            // 下の壁のY軸の下限
            let pan_lowest_y = UInt32( center_y - panTexture.size().height / 2 -  random_y_range / 2)
            // 1〜random_y_rangeまでのランダムな整数を生成
            let random_y = arc4random_uniform( UInt32(random_y_range) )
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
            let pan_y = CGFloat(pan_lowest_y + random_y)
            
            // 下側の壁を作成
            let Pan = SKSpriteNode(texture: panTexture)
            Pan.position = CGPoint(x: 0.0, y: pan_y)
            
            // スプライトに物理演算を設定する
            Pan.physicsBody = SKPhysicsBody(rectangleOf: panTexture.size())    // ←追加
            
            // 衝突の時に動かないように設定する
            Pan.physicsBody?.isDynamic = false    // ←追加
            Pan.physicsBody?.categoryBitMask = self.panCategory    // ←追加
            
            pannode.addChild(Pan)
            
            let itemScoreNode = SKNode()
            itemScoreNode.position = CGPoint(x: Pan.size.width / 2, y: pan_y)
            itemScoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: Pan.size.width, height: Pan.size.height))
            itemScoreNode.physicsBody?.isDynamic = false
            itemScoreNode.physicsBody?.categoryBitMask = self.panCategory
            itemScoreNode.physicsBody?.contactTestBitMask = self.birdCategory
            
            pannode.addChild(itemScoreNode)
            
            pannode.run(panAnimation)
            
            self.panNode.addChild(pannode)
        })
        
        // 次の壁作成までの待ち時間のアクションを作成
        let waitPanAnimation = SKAction.wait(forDuration: 4)
        let waitMomentAnimation = SKAction.wait(forDuration: 1)
        
        // 壁を作成->待ち時間->壁を作成を無限に繰り替えるアクションを作成
        let repeatForeverAnimation = SKAction.repeatForever(SKAction.sequence([createPanAnimation, waitPanAnimation]))
        let PanAnimation = SKAction.sequence([waitMomentAnimation,repeatForeverAnimation])
    ...
    func playSound() {
        do {
            ...
            let asset = NSDataAsset(name:"item")
